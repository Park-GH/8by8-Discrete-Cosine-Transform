...
void _2D_8x8_DCT() {
	...
	int pheight, pwidth;
	cin >> str;
	cout << "Input width and height : ";	// (raw파일이 width height 순서였다..)
	cin >> height >> width;
	pheight = ceil(height / (double)BLK_SIZE) * BLK_SIZE;
	pwidth = ceil(width / (double)BLK_SIZE) * BLK_SIZE;
	clock_t start = clock();
	...
	cout << "Execution time : " << (double)1000 * (clock() - start) / CLOCKS_PER_SEC << "msec" << endl;
	ImgIn.close();
	ImgOut.close();
}
